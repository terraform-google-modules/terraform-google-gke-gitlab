...
resource "google_sql_user" "gitlab" {
  ...
  password = var.gcp_existing_db_secret_name != "" ? data.google_secret_manager_secret_version.secret_managed_postgres_predefined_pass[0].secret_data : google_secret_manager_secret_version.secret_managed_postgres_random_pass[0].secret_data
}
