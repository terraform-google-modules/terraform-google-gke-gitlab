...
data "google_secret_manager_secret_version" "secret_managed_postgres_predefined_pass" {
  secret    = var.gitlab_dbpassword_secret_name
  project   = var.project_id
  count     = var.gitlab_dbpassword_secret_name != "" ? 1 : 0
}

# Create a secret in GCP secret manager with a random value for the DB passeod
resource "google_secret_manager_secret" "secret_managed_postgres_random_pass" {
  secret_id = "gitlab-db-password"
  replication {
    automatic = true
  }
  count = var.gitlab_dbpassword_secret_name != "" ? 0 : 1
}

# Fill the secret created with a random string
resource "google_secret_manager_secret_version" "secret_managed_postgres_random_pass" {
  secret      = google_secret_manager_secret.secret_managed_postgres_random_pass[0].id
...
resource "google_sql_user" "gitlab" {
  ...
  password = var.gitlab_dbpassword_secret_name != "" ? data.google_secret_manager_secret_version.secret_managed_postgres_predefined_pass[0].secret_data : google_secret_manager_secret_version.secret_managed_postgres_random_pass[0].secret_data
}
